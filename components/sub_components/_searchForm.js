const SearchForm = ({ active }) => {
    return (
        <form className={`search-form-container flex w-[90%] [top:115%] absolute left-[5%] rounded-lg lg:w-auto lg:static group transition-all ease-linear delay-500 ${active ? '' : '[transform:translateY(-10rem)]'} lg:translate-y-0 [transform-origin:0_0] sm:w-[70%] sm:left-[15%]`} method="get">
            ...
            <button className="bg-main-color text-white pt-[0.3rem] pb-[0.1rem] px-3.5 rounded-r-lg transition-all ease-linear delay-100" type="submit" aria-label="Click to search">
